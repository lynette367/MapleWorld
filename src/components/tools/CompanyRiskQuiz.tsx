...
const questions: Record<string, Question> = {
    q1: {
        ...
        text: "Q1 你现在的身份阶段？",
        options: [
            { label: "F1 在读（未开始 CPT / OPT）", value: "f1", next: "q2" },
            { label: "CPT", value: "cpt", next: "result_cpt" },
            { label: "OPT / STEM OPT", value: "opt", next: "q2" },
            { label: "已转 H1B 或绿卡", value: "authorized", next: "result_authorized" },
        ],
    },
    q2: {
        id: "q2",
        ...
        hint: "这是判断风险高低最关键的一步。",
        options: [
            { label: "只出资，不参与经营（纯被动股东）", value: "passive", next: "q3" },
            { label: "参与日常经营管理", value: "active_manage", next: "result_active" },
            { label: "自己给公司干活，并从中拿工资/报酬", value: "active_paid", next: "result_active" },
        ],
    },
    q3: {
        id: "q3",
        ...
        options: [
            { label: "完全不发钱，只是登记股东", value: "no_pay", next: "result_passive_low" },
            { label: "会发工资或顾问费给我", value: "pay", next: "result_active" },
        ],
    },
};

type Result = {
    level: "low" | "medium" | "high" | "na";
    levelLabel: string;
    title: string;
    description: string;
...
const results: Record<string, Result> = {
    result_cpt: {
        ...
        description:
            "CPT 的工作授权和你就读学校的课程要求、雇主实习安排绑定得很紧。如果你打算以自己名下的公司作为 CPT 雇主，或者用自己开的公司满足 CPT 要求，这类操作极容易被认定为不符合 CPT 授权范围，进而影响 F1 身份本身。这不是税务问题，是身份问题，建议在采取任何行动前先找学校 DSO 或移民律师书面确认。",
        nextSteps: [
            { label: "查看开公司身份合规完整指南 →", href: "/compliance/can-f1-open-company" },
        ],
    },
    result_authorized: {
        level: "na",
        ...
        description:
            "本自测主要针对 F1 / CPT / OPT 阶段的身份合规风险。H1B 有自己的雇主绑定规则，绿卡持有者在开公司这件事上限制则少得多。你面对的是不同的规则体系，建议单独咨询移民律师，而不是套用本工具的判断。",
        nextSteps: [],
    },
    result_active: {
        level: "high",
        ...
        description:
            "只要你在公司里做的事超出了「单纯出资」的范围——不管是参与日常经营，还是自己给公司干活并拿钱——都有可能被认定为在从事工作授权之外的雇佣活动。F1 / OPT 的工作授权通常要求雇主明确、岗位明确、且与你的专业相关。自己开公司给自己发工资，恰恰是这几条最容易踩线的地方。",
        nextSteps: [
            { label: "查看开公司身份合规完整指南 →", href: "/compliance/can-f1-open-company" },
        ],
    },
    result_passive_low: {
        ...
        description:
            "只出资、不经营、不领取任何形式的报酬，通常被视为被动投资行为，和「工作」是两回事，这条路径的合规风险相对较低。但「相对低」不等于「零风险」——建议保留完整的书面记录（股东协议、不参与经营的书面说明），并在正式操作前找移民律师做一次确认，尤其是涉及资金来源和股权结构的部分。",
        nextSteps: [
            { label: "查看开公司身份合规完整指南 →", href: "/compliance/can-f1-open-company" },
        ],
    },
...
export default function CompanyRiskQuiz() {
    ...
    const levelStyles: Record<Result["level"], string> = {
    ...
    return (
        <div className="my-8 rounded-[28px] border border-brand-pink/40 bg-white/95 p-6 sm:p-8 shadow-[0_20px_60px_rgba(244,185,200,0.25)]">
            ...
            {!isResult ? (
                ...
                <div className={`rounded-2xl border p-5 ${levelStyles[result.level]}`}>
                    ...
                    <p className="mt-4 text-xs text-brand-ink/50 leading-relaxed">
                        本结果为一般性风险提示，不构成移民律师意见。涉及身份合规问题，请务必咨询有资质的移民律师或学校 DSO。
                    </p>
                </div>
            )}
        </div>
